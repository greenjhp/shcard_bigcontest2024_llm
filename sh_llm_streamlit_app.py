...
# 임베딩 로드
embeddings = np.load('embeddings_array_file.npy')

def generate_response_with_faiss(question, df, embeddings, model, embed_text, time, local_choice, index_path='faiss_index.index', max_count=10, k=3, print_prompt=True):
    filtered_df = df
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            response = generate_response_with_faiss(prompt, df, embeddings, model, embed_text, time, local_choice, index_path='faiss_index.index', max_count=10, k=3, print_prompt=True)
